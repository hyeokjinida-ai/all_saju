...
// 자동 모드(auto.html): CTA가 자동 사주 플로우를 시작
// 기본(index.html): CTA가 신청 페이지(apply.html)로 이동 → 거기서 카카오 연결
const onKakao = () => {
  if (typeof window !== 'undefined' && window.__autoFlowStart) { window.__autoFlowStart(); return; }
...
function KakaoButton({ children = '카카오톡으로 상담받기', sub }) {
  // 자동 모드: 골드 버튼 + '내 사주 바로 보기'
  if (typeof window !== 'undefined' && window.__autoMode) {
    return (
      <button onClick={onKakao} style={{
        width: '100%', minHeight: 58,
        background: 'linear-gradient(180deg, #e8c878 0%, #d4af6a 100%)',
        color: 'var(--wine-deep)', border: 'none', cursor: 'pointer',
        fontFamily: "'Noto Serif KR', serif", fontWeight: 700, fontSize: 16,
        letterSpacing: '0.25em',
        display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12,
        boxShadow: '0 0 24px rgba(212,175,106,0.3)',
      }}>
        <span>내 사주 바로 보기</span>
        <span className="brush" style={{ fontSize: 20, letterSpacing: 0 }}>展</span>
      </button>
    );
  }
  return (
...
function ProblemSection() {
  ...
  return (
    <section className="scene-wine" data-screen-label="05 Problem" style={{
      position: 'relative', overflow: 'hidden', padding: '0 0 70px',
    }}>
      {/* full-bleed mountain — absolutely positioned, dissolves into wine */}
      <div style={{
        position: 'absolute', top: 0, left: 0, right: 0, height: 480,
        pointerEvents: 'none', zIndex: 0,
      }}>
        <window.MountainScene height={480} withLantern={true}/>
        {/* one smooth dissolve — clear at top, wine at bottom */}
        <div style={{
...
function PrecisionSection() {
  ...
  return (
    <section className="scene-cosmos" data-screen-label="07 Precision" style={{
      position: 'relative', padding: '80px 24px 70px', overflow: 'hidden',
      textAlign: 'center',
    }}>
      <div style={{ position: 'relative', zIndex: 1 }}>
        <window.SectionLabel index="六" hanja="精" label="정밀한 산출"/>
        ...
        <div className="myeongjo" style={{
          fontSize: 11, color: 'var(--bone-faint)', letterSpacing: '0.15em', marginBottom: 20,
        }}>
          ...
          {[
            ...
            <div key={i} style={{ display: 'flex', gap: 16, alignItems: 'flex-start', padding: '11px 0' }}>
              <div style={{
                ...
                border: '1px solid var(--gold)', background: 'var(--wine-deep)',
