...
export default async function CheckoutPage({
  ...
  // 정가(취소선) — 0010 컬럼이라 따로 읽고, 없으면 조용히 할인 줄을 뺀다.
  ...
  const hasDiscount = compareAt != null && compareAt > order.amount;
  const off = hasDiscount ? compareAt! - order.amount : 0;
  const pct = hasDiscount ? Math.round((off / compareAt!) * 100) : 0;
    ...
    <div className={`${world} min-h-screen`} style={{ background: bg }}>
      <div className="mx-auto max-w-md px-5 py-10">
        ...
        <div className="mt-6 divide-y divide-hairline border-y border-hairline">
          <Row k={name} sub={desc[slug]} v={hasDiscount ? `${formatKRW(compareAt!)}` : formatKRW(order.amount)} />
          {hasDiscount && (
            <Row
              k={
                <>
                  지금 결제 시 할인 <span className="ml-1 text-[12px] text-bone-faint">-{pct}%</span>
                </>
              }
              v={`-${formatKRW(off)}`}
            />
        ...
        <div className="mt-7">{widget({ ctaLabel: `${formatKRW(order.amount)} 결제하기` })}</div>
