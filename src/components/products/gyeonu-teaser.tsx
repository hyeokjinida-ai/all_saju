...
export type GyeonuCutId =
  ...
  // 상품부 전용(2026-09-06) — 자리마다 새로 뽑은 연기컷.
  | "g-twokinds"; // 펼친 장부의 두 자리를 짚는다 — 열두 달 격자 머리
...
import { GyeonuBubble, GYEONU_SAY_BOX, SAY_ART, type SayTail } from "@/components/products/gyeonu-bubble";
 ...
 *
 *   슬롯            위 숨   글    아래 숨   합    자리
 *   1줄·앞 컷 소유    52    30     110     192   강 → 「그날, 두 사람은 갈라졌습니다」 → 강가
  ...
  return (
    ...
    >
      ...
      <WebtoonPanel
        ...
        align="left"
      />
      {/* 여기 있던 「방금 그 대목에서 / 눈이 오래 머무셨네요」는 뺐다(GPT 대조 진단 2026-09-06).
          바로 앞 응시 컷의 「○○님, 아직 강 건너를 보고 계셨군요」와 **같은 일**을 한다 —
          둘 다 「제가 당신을 보고 있습니다」다. 두 번 하면 통찰이 아니라 행동 추적으로 읽힌다.
          컷은 남긴다: 카드가 답을 준 직후의 **무언 반응**이라 침묵이 대사보다 낫다
          (이 판은 이미 DotRest 로 침묵을 쓴다). 재확인 1회 → 결과 → 표정 변화 1회면 충분하다. */}
      ...
      <WebtoonPanel
        ...
        say={{
          ...
          // 「펼쳐 드립니다」(제공) → 「같이 봅니다」(동행). 거래 조건은 바로 아래 UI 가 맡는다.
          lines: ["여기부터는,", "복채를 받고 같이 봅니다."],
        }}
      />
...
export function ReunionCalendar({ data, name }: { data: Reunion; name: string }) {
  ...
  return (
    <section>
      ...
      {/* 절단 뒤 상품부의 **첫 그림**. 이 자리 전용으로 뽑은 컷이다 —
          펼친 장부의 두 자리를 양손 검지로 짚는다(위 머리 「연락할 때와 기다릴 때를 나눠 봅니다」를
          그림이 그대로 받는다). 대사는 안 붙인다: 바로 위 두 줄이 이미 그 말이고,
          아래 열두 칸이 실물로 보여준다 — 세 번 말하면 잡음이다.
          ⚠ 랜딩에는 안 쓴다(GPT 대조 진단: 랜딩에선 카드 둘·격자가 같은 정보를 이미 준다). */}
      <GyeonuCut id="g-twokinds" alt="펼친 장부의 두 자리를 손가락으로 짚는 견우" gap={28} />
...
const TOC_REUNION: { title: string; items: string[] }[] = [
  ...
  { title: "2장. 두 사람이 갈라진 날", items: ["그 무렵 흐름은 어땠는지", "그래서 왜 내 탓이 아닌지"] },
  ...
  { title: "9장. 다시 만나지 않는다면", items: ["그 사람이 아니어도 되는 이유", "다음에 올 사람의 결과 나이대"] },
 ...
 *
 *  `data` 를 받는 이유: 환승(다시 만나지 않는다면)이 여기로 들어왔기 때문이다.
...
export function ReunionToc({ data }: { data?: Reunion }) {
  return (
    <section className="mt-14">
      ...
      {/* 「보시죠」(권유·세일즈맨 톤)를 빼고 **「같이」**만 남겼다 — 견우의 친밀도는
          「저도 압니다 / 같이 봅니다」가 상한이다(2026-09-06). */}
      <GyeonuCut
        id="g-face-back"
        alt="별길에서 돌아보는 견우"
        gap={40}
