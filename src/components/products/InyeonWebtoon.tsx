...
function TocCard({ priceLabel, compareLabel }: { priceLabel: string; compareLabel?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* 분량 앵커(A4 몇 쪽·몇 분)는 형님 지시로 걷어냈다(2026-09-02) — 우리 숫자를 꺼내는
          순간 2~5만 자로 파는 경쟁과 같은 자에서 재게 된다. 분량은 말하지 않는다. */}
...
const FAQ: [string, string][] = [
  ...
  [
    "뭘 얼마나 받나요?",
    "10개 챕터예요. 점수 / 걸어온 길 / 반복되는 패턴 / 만나는 달 세 개 / 내게 올 사람 / 알아보는 신호 셋 / 조심할 달 / 크게 바뀌는 해 / 내 고민 답 / 이번 주에 할 것 3가지. 위 목차에 그대로 적어뒀어요.",
  ],
  [
