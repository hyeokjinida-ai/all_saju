...
function TeaserStep({
  ...
  return (
    ...
    >
      ...
      {/* 48 → 56: 이 블록의 **마지막이 등불 컷**으로 바뀌었다(2026-09-05).
          컷 대사 뒤에 오는 블록은 티저 전체에서 56 을 쓴다(g-river→달력, p-farshore→T3) —
          여기만 48 이면 아홉 컷 중 한 자리만 리듬이 짧아진다.
          ⚠ 인라인인 이유: `mb-14` 는 이 리포에 없던 클래스라 dev 의 Tailwind 가 생성을
      ...
      {/* 까치 컷 — 바로 아래 잠긴 줄(「연락해도 되는 달」·「다리가 놓이는 달」)의 **그림 예고**다.
          잠금 목록은 글로만 보면 「안 주는 것 목록」으로 읽힌다(2026-08 실측에서 이미 한 번 데였다).
          서찰을 물고 강을 건너오는 까치를 앞에 세우면 그 목록이 「오는 것 목록」으로 뒤집힌다.
          ⚠ 달을 말하지 않는다 — 「따로 있습니다」에서 끊는 게 이 컷의 일이다. */}
      {isReunion && teaser?.reunion && (
        // 아래 잠금 목록이 mt-4(16) 로 시작한다 — 공용 블록이라 못 건드린다.
        // 여기서 40 을 더해 「컷 대사 → 다음 블록 56」 리듬을 맞춘다(g-river→달력과 같은 눈금).
        <div style={{ marginBottom: 40 }}>
          <GyeonuCut
            id="p-magpie"
            alt="발목에 서찰을 묶고 은하수를 건너 나는 까치"
            say="소식이 건너오는 달이… 따로 있습니다."
          />
        </div>
      )}
      ...
      {teaser && (
        <>
          ...
          {isReunion && teaser.reunion && (
            <>
              {/* 정점 — 오작교. 티저에서 제일 큰 그림이고 유일하게 **아무것도 안 가린 약속**이다.
                  앞 여백 56(정점 앞 큰 숨): 잠금 목록에 붙여 두면 목록의 삽화로 읽힌다.
                  대사는 사실만 말한다 — 「다시 이어집니다」로 단정하면 재회가 안 되는 손님에게
                  거짓말이 되고, 바로 아래 T4(강을 건너지 않는다면)와도 어긋난다.
                  달 이름은 여기서 말하지 않는다: 그게 다음 검정 판이 끊는 자리다. */}
              <GyeonuCut
                id="p-bridge"
                alt="수백 마리 까치가 은하수 위로 다리를 이룬 밤"
                gap={56}
                say="다리가 놓이는 달에는, 강이 이렇게 됩니다."
              />
              {/* 정점 뒤 한 박자 — **정면 대면 컷**이다. 돈 얘기는 마주 보고 한다(산군이
                  teaser-face 를 결제 전환 자리에 쓰는 것과 같은 판단).
                  gap 44(기본): 앞 정점과 뒤 검정 판이 각각 56 을 쓰므로 이 컷까지 56 이면
                  56 이 세 번 연달아 나와 절단의 「큰 숨」이 평범해진다. 큰 숨은 정점 앞과
                  절단 앞 둘뿐이다(ReunionCut 의 mt-14 가 뒤쪽 56).
