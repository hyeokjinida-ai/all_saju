...
function readRival(
  ...
  return {
    basis: "나",
    strength,
    // ⚠ 첫 줄에 있던 「그 사람 생일을 안 받아서 직접 못 봤습니다」는 뺐다(2026-09-06).
    //    이 카드는 결제 직전 정점 자리인데 **첫 문장이 「이 결과는 불완전합니다」**로 열려
    //    구매 확신을 깎았다(GPT 대조 진단). 순서를 결과 → 의미 → 한계로 바꾼다:
    //    한계는 카드 맨 아래 13px 각주(NightRival)가 이미 말하고, 결과지 프롬프트도
    //    「근거: 내 곁자리 흐름」 + 「생년월일을 안 받았다, 아는 척하지 말 것」을 따로 받는다.
    lines: [
