  ...
  const turningYear =
    ty && ty.year
      ? {
          year: ty.year,
          age: ty.age,
          // 연도를 이 줄에 다시 쓰지 않는다 — 바로 위에 붓 동그라미까지 친 큰 숫자가 있어서,
          // 넣으면 같은 연도를 2초 안에 두 번 읽힌다. 큰 숫자가 "언제"를 맡고 이 줄은 "무슨 일"을
          // 맡는 분업이라야 카드가 한 문장으로 읽힌다(형님이 오늘 세 번 잡은 중복과 같은 유형).
          line:
            voice === "sangun"
              ? isThisYear
                ? `바로 올해다. ${ty.age}세에 한 번 크게 갈린다.`
                : `너는 ${ty.age}세에 한 번 크게 갈린다.`
              : isThisYear
                ? `바로 올해예요. ${ty.age}세에 한 번 크게 갈리세요.`
                : `${ty.age}세에 한 번 크게 갈리세요.`,
        }
      : null;
